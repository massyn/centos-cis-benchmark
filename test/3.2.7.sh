...
grep "net.ipv4.conf.all.rp_filter" /etc/sysctl.conf | grep -E "net.ipv4.conf.all.rp_filter = 1"
if [[ $? == 0 ]]; then
        exit 0
fi
...
grep "net.ipv4.conf.default.rp_filter" /etc/sysctl.conf | grep -E "net.ipv4.conf.default.rp_filter = 1"
if [[ $? == 0 ]]; then
        exit 0
fi
