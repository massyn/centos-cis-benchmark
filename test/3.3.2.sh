...
grep "net.ipv6.conf.all.accept_redirects" /etc/sysctl.conf | grep -E "net.ipv6.conf.all.accept_redirects = 0"
if [[ $? == 0 ]]; then
        exit 0
fi
...
grep "net.ipv6.conf.default.accept_redirects" /etc/sysctl.conf | grep -E "net.ipv6.conf.default.accept_redirects = 0"
if [[ $? == 0 ]]; then
        exit 0
fi
